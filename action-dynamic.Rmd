...
server <- function(input, output, session) {
  observeEvent(input$min, {
    updateSliderInput(input = "n", min = input$min)
  })  
  observeEvent(input$max, {
    updateSliderInput(input = "n", max = input$max)
  })
}
```
...
The update functions look a little different to other Shiny functions: they all take name of the input (as a string) as the the `input` argument[^action-dynamic-2].
...
server <- function(input, output, session) {
  observeEvent(input$reset, {
    updateSliderInput(input = "x1", value = 0)
    updateSliderInput(input = "x2", value = 0)
    updateSliderInput(input = "x3", value = 0)
  })
}
```
...
server <- function(input, output, session) {
  observeEvent(input$n, {
    ...
    updateActionButton(input = "simulate", label = label)
  })
}
```
...
server <- function(input, output, session) {
  ...
  observeEvent(territory(), {
    ...
    updateSelectInput(input = "customername", choices = choices) 
  })
  ...
  observeEvent(customer(), {
    ...
    updateSelectInput(input = "ordernumber", choices = choices)
  })
...
There's an important issue we need to discuss if you want to use the update functions to change the current `value`[^action-dynamic-3] of an input.
...
[^action-dynamic-3]: This is generally only a concern when you are changing the `value`, but be some other parameters can change the value indirectly.
...
server <- function(input, output, session) {
  observeEvent(input$n,
    updateNumericInput(input = "n", value = input$n + 1)
  )
}
```
...
server <- function(input, output, session) {
  observeEvent(input$temp_f, {
    ...
    updateNumericInput(input = "temp_c", value = c)
  })
  
  observeEvent(input$temp_c, {
    f <- round((input$temp_c * 9 / 5) + 32)
    updateNumericInput(input = "temp_f", value = f)
  })
}
```
...
server <- function(input, output, session) {
  observeEvent(input$controller, {
    updateTabsetPanel(input = "switcher", selected = input$controller)
  })
}
```
...
server <- function(input, output, session) {
  observeEvent(input$dist, {
    updateTabsetPanel(input = "params", selected = input$dist)
  }) 
...
Note that note the value of (e.g.) `input$mean` is independent of whether or not its visible to the user.
...
6.  (Hard) Make a wizard that allows the user to upload their own dataset.
