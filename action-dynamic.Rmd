...
[^action-dynamic-1]: Note that I've used `observeEvent()` here, although `observe()` would also work and would be more concise.
...
There's an important issue we need to discuss if you want to use the update functions to change the current `value`[^action-dynamic-2] of an input.
From Shiny's perspectve, using an update function to modify `value` is no different to the user modifying the value by clicking or typing.
...
[^action-dynamic-2]: This is generally only a concern when you are changing the `value`, but be some other parameters can change the value indirectly.
...
It contains a single input control and a observer that increments its value by one.
...
For example, imagine that you want to create a temperatue conversion app where you can either enter the temperature in Celsius or in Fahrenheit:
...
You can also use this idea to a "wizard", a type of interface that makes it easier to collect a bunch of information by spreading it across multiple pages.
...
-   You use `uiOutput()` to insert a placeholder in your user interface.
    This code is run when your app launches and it leaves a "hole" that your server code can later fill in.
...
It dynamically creates a different type of input control depending on an input.
...
This is one reason that selectively showing and hiding UI is a better approach if works for you --- because you're not destroying and recreating the controls, you don't need to do anything to preserve the values.
...
We only want it to change when `input$type` or `inpu$label` changes.
...
-   To generate the output, I need to use use a new trick.
...
The `%||%` (default value for `NULL`) operator is documented [here](https://rlang.r-lib.org/reference/op-null-default.html).
