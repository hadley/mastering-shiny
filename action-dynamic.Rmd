...
You're unlikely to create such an obvious problem in your own app, but it can crop up if you are updating multiple controls that depend on one another, as in the next example.
...
There's no way around this problem because you have one idea (the temperature) with two expressions in the app (Celsius and Fahrenheit). Here we are lucky that cycle quickly converges to a value that satisfies both constraints. In general, you are better off avoiding these situations, unless you are willing to very carefully analyze the convergence properties of the underlying dynamic system that you have created.
...
1.  Complete the user interface below with a server function that updates
    ...
    also change the label from "County" to "Parish" for "Louisiana" and 
    "Borough" for "Alaska".
...
1.  Extend the previous app so that you can also choose to select all 
    continents, and hence see all countries. You'll need to add `"(All)"` to 
    the list of choices, and then handle that specially when filtering.
