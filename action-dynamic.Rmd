...
Take this very simple example. The app has two inputs that control the range (the `min` and `max`) of another input, a slider. The key idea is to use `observeEvent()`[^observe] to trigger `updateNumericInput()` whenever the `min` or `max` inputs change. 
...
demo$resize(350)
demo$screenshot("onload")
demo$set_values(max = 4)
demo$screenshot("max-increase")
demo$set_values(min = -1)
demo$screenshot("min-decrease")
demo$deploy()
```

`r demo$figure()` shows the app in action.
...
The simplest uses of the update functions are to provide small conveniences for the user. For example, maybe you want to make it easy to reset parameters back to their initial value. The following snippet shows how you might combine an `actionButton()`, `observeEvent()` and `updateNumericInput()`:
...
```{r update-reset, fig.cap = demo$caption(), out.width = "33%", fig.show = "hold", fig.align = "default", echo = FALSE, message = FALSE, cache = FALSE}
...
demo$screenshot("set")
demo$click("reset")
demo$screenshot("reset")
demo$deploy()
```

You can play with it yourself at `r demo$link()`.
...
```{r update-button, fig.cap = demo$caption(), out.width = "33%", fig.show = "hold", fig.align = "default", echo = FALSE, message = FALSE, cache = FALSE}
demo <- demo_inline("update-button", ui, server)
demo$resize(350)
demo$screenshot("onload")
demo$set_values(n = 1)
demo$screenshot("set1")
demo$set_values(n = 100)
demo$screenshot("set100")
demo$deploy()
```

You can see this app in action at `r demo$link()`.
...
```{r update-temperature, out.width = "33%", fig.show = "hold", fig.align = "default", echo = FALSE, message = FALSE, cache = FALSE}
...
You can try this out yourself at `r demo$link()`.
...
```{r dynamic-panels, fig.cap = demo$caption(), out.width = "33%", fig.show = "hold", fig.align = "default", echo = FALSE, message = FALSE, cache = FALSE}
demo <- demo_inline("dynamic-panels", ui, server)
demo$resize(300, 220)
demo$screenshot()
demo$set_values(controller = "panel2")
demo$screenshot("panel2")
demo$set_values(controller = "panel3")
demo$screenshot("panel3")
demo$deploy()
```
...
```{r dynamic-conditional, fig.cap = demo$caption(), out.width = "50%", fig.show = "hold", fig.align = "default", echo = FALSE, message = FALSE, cache = FALSE}
demo <- demo_inline("dynamic-conditional", ui, server)
demo$resize(800)
demo$screenshot("normal")
demo$set_values(dist = "uniform")
demo$screenshot("uniform")
demo$deploy()
```
...
(I've removed the `min` and `max` arguments to )
...
If you run this app, you'll discover a really annoying behaviour: whenever you change the number of colours, all the data you've entered disappears. We can fix this problem by using the same technique as before: setting `value` to the (isolated) current value. I'll also tweak the appearance to look a little nicer, including displaying the selected colours in a plot.
...
From there, it's a simple generalisation to work with any data frame. Here I'll illustrate it using the data frames in the datasets package, but you can easily imagine how you might extend this to user uploaded data.
...
```{r filtering-final, fig.cap = "A dynamic user interface automatically generated from the fields of the selected dataset", out.width = "100%", fig.show = "hold", fig.align = "default", echo = FALSE, message = FALSE, cache = FALSE}
...
`r demo$figure()` shows the app in action, and you can play with it yourself at `r demo$link()`.
