...
Currently[^tab-future] you need a smidgen of CSS to make this technique work: `tags$style("#switcher { display:none; }")`. If you adapt this for your own code make sure to repace `#switcher` with `#` followed the `id` of your `tabsetPanel()`.
...
* I create the text boxes by using `map()` to create one `textInput()` for 
  each input in `col_names()`. `output$col <- renderUI()` inserts these text boxes
...
This code uses rlang's infix `%||%` operator, which returns the user-supplied default value on the operator's right-hand side (e.g., the empty string `""` in the above code) when the left-hand side (e.g., `input[[.x]]` in the above code) is `NULL`.
