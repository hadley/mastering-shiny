...
level of sophistication is a bit of a trick: you can make your UI complex and still responsive by selectively showing and hiding part of it. The easiest way to selectively display part of a UI is to slightly abuse the `tabetPanel()` UI. Currently[^tab-future] you need a dash of css to make it work: in the following UI specification `tags$style("#switcher { display:none; }")` hides the tab switcher. If you adapt this for your own code make sure to repace `switcher` with id of your the `tabsetPanel()`.
...
There are many creative uses for this basic ideas. The following code shows you how might create a "wizard", a type of interface that makes it easier to collect a lot of information by breaking it down into a progression of simple steps. 
