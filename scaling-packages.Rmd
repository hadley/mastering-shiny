...
In the long run, it also helps you document the pieces of complex apps using [roxygen2](https://roxygen2.r-lib.org), although we won't discuss that in this book.
...
-   If it doesn't exist already, create an `R` directory and move `app.R` into it.

-   You'll then transform the app into a standalone function by wrapping
...
-   If you have any calls to `source()`, you can now remove them as the package load process will take care of them for you.
...
-   If you are loading datasets using `read.csv()` or similar, you can instead use `usethis::use_data(mydataset)` to save the data in the `data/` directory.
    The package load process will automatically make this data available to you.
...
-   If you don't already have one, create an RStudio project file by calling `usethis::use_rstudio()`.

-   Restart RStudio.

To make this concrete, we'll next work through a simple case study before coming back to the benefits of this work in Section \@ref(package-workflow).
...
Even this simple change is useful because it helps me understand the big picture of the app.
If instead I want to dive into the details, I can look at the code for the individual modules.
...
As an optional extra, I convert `birthstones.csv` to a package dataset by running `usethis::use_data("birthstones")`.
This creates `data/birthstones.rda`, which will be loaded automatically when loading the package.
...
The easiest was to find and open your `.Rprofile` is to run `usethis::edit_r_profile()`.
...
Shinyapps.io and RStudio connect are great way to share apps with folks who aren't familiar with R.
But sometimes you want to share apps with your colleagues who do use R --- maybe instead of allowing the user to upload a dataset, you want to provide them with a function that they call with a dataset.
...
Now, whenever you ready to share an update version of your app with your users, you can run `rsconnect::deployApp()`.

### `R CMD check`

A minimal package contains an `R/` directory, a `DESCRIPTION` file, and a function to run your app.
This is already useful because it unlocks some useful workflows to speed up app development.
In the next chapter, we'll see the most powerful workflows --- the ability to do automated testing.
But there's one other technique you might want to use: the automated checks provided by `R CMD check`.

I think this is what makes a serious package --- you care enough about the package conventions to repeatedly use `Cmd/Ctrl + Shift + E` to call `R CMD check` and fix all the problems that you find.
...
Before you make your first full app-package, you'll need to read "[The whole game](https://r-pkgs.org/whole-game.html)" chapter of *R packages*: it will give you a fuller sense of the package structure, and introduce you to other useful workflows.
...
-   Pick a license and then use the appropriate `use_license_` function to put it in the right place.
    ...
    See <https://r-pkgs.org/license.html> for more details of licensing.
...
-   If your app contains small reference datasets, put them in `data` or `inst/extdata`.
    We discussed `usethis::use_data()` above --- it's nice because it's stored in `.rda` format, which is faster to load, and is loaded lazily so if it's not used by the app, it's not loaded in memory.
...
A package is just a lightweight set of convention that unlock useful tools and workflows.
In this chapter, you learned how to turn an app into a package and some of the reasons you might want to.
In the next chapter, you'll learn more about what I think is the most important reason to turn your app into a package: to make it easier to test.
