...
One caveat. `birthstoneServer` function can't find the variable `stones` no longer in the environment in which it is defined. So, it searches `stones` in the base package before in `app.R`. As `stones` is not in the base package, it is OK. However, if the variable name is `units`, we shall see "Error: object of type 'closure' is not subsettable".
