...
```{r}
ageUI <- function() {
  dateInput("birthday", "When were you born?", max = Sys.Date())
}
ageServer <- function(input, output, session) {
  reactive({
...
server <- function(input, output, session) {
  age <- ageServer(input, output, session)
  output$age <- renderText(age())
}
```

The problem with this attempt is that it's not isolated from the rest of the app, and because the name of the reactive input is concealed inside a function, it's easy to create a problem:

```{r}
ui <- fluidPage(
  ageUI(),
  textInput("birthday", "What would you like for your birthday?"),
  textOutput("age")
)
```

Debugging the problem that this creates will be painful because it will reveal itself through failure of reactivity - the output won't upate as you expect, or you'll get weird errors because two controls are fighting for the same input value. 
...
To make the above code a module, we need to make two changes. First we need to add an `id` argument to our UI component, and use it with special `NS()` function. `NS` is short for namespace: it creates a "space" of "names" that is unique to the module. 

```{r}
ageUI <- function(id) {
  dateInput(NS(id, "birthday"), "When were you born?", max = Sys.Date())
}
```

We now need to specify and id when creating our UI. This is important because it puts this id in the same place as all the others. Making the same mistake as earlier would be much harder.
...
Then instead of calling our age server function directly we call it with `callModule()` passing along the id:

```{r}
server <- function(input, output, session) {
  age <- callModule(ageServer, "birthday")
  output$age <- renderText(age())
}
```

`callModule()` automatically tweaks the `input` and `output` so it looks for `birthday` inside the `birthday` namespace. You can think if it as doing `input$birthday$birthday` (but it's actually `input$birthday-birthday`).

### Namespaces

The key idea of modules is that namespace: isolating ids so that they're nested in a unique namespace that doesn't interfere with anything else.
...
Need example here that illustrates the isolation.
