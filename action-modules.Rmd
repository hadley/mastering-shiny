...
We'll start by create a module that allows the user from built-in data frames. This isn't terribly useful in its own right, but it's a useful building block for more complex apps. I add one argument to the module UI to make it possible to select built in datasets that are (e.g.) either data frames (`filter = is.data.frame`) or matrices (`filter = is.matrix`). This is minor enhancement to code that you first saw in Section \@ref(adding-ui).
...
The module server is similarly simple. There's only one new idea here, like a function and unlike a regular server function, most modules return a value. Like regular functions, the return value is the last expression processed in the function; but here it will almost always be a reactive:
...
To use a module server that returns a value, you use `<-` to capture its value in your app server function. You can see that in our module function: we create a new reactive variable `data` that is then used to generate output.
...
I've made a few design decisions:

* The module function takes an argument which is passed along to the module UI.
...
* I use a tabular output to look at the data. It doesn't really matter what 
  ...
  check that the app sdoes what I expect.
...
One challenge with the previous app is there's no easy way to label the histogram with the name of the variable. We could modify the module to return the name instead of the value, but then the module user would have to do all the subsetting. Instead we could return _both_ the name and the value:
...
The key idea is simple: instead of returning one value, we return a list of reactives.

To use this with `histogramServer()` we need to add an additional argument, which will become the plot title. This needs to be reactive (since it will change over the lifetime of the app), so we check for it. Also notice the default value: that too must be a reactive value so we create a "constant" reactive.
...
If you find yourself using the pattern a lot, you might also look into the zeallot package which provides a `%<-%` operator that allows you to assign into multiple variables (sometimes called multiple, unpacking, or destructuring assignment).
