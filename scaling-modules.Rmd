...
Next I write a function to generate a page of the wizard. This includes contain a "title" (not shown, but used to identify the page for switching), the contents of the page (supplied by the user), and the two buttons[^3].
...
-   The first page doesn't have a previous button. Here I use a trick that `if` returns `NULL` if the condition is `FALSE` and there no else block.
...
Unfortunately we need to repeat ourselves slightly when using the module: need to make sure that `n` argument to `wizardServer()` is consistent with the `pages` argument to `wizardUi()`. This is a principled limitation of the module system which we'll discuss in more detail in Section \@ref(module-objects).
...
We'll finish up with a case study that uses dynamic UI (the topic of Chapter \@ref(action-dynamic)). This forces you to figure out whether you're in a UI context where the namespace `id` is used explicitly in a `NS()` call, or in a server context where namespace `id` handled automatically.

This example is derived from a question in Shiny community: <https://community.rstudio.com/t/67898.> I've simplified it as much as possible for educational purposes, so you might want to read the original post for a more realistic example. The goal of this module is to provide an "advanced settings" dialog box that lets you control settings that aren't important enough to take up space in the main UI. We're going to make the dialog dynamic, so we can reuse the same code for different datasets.

```{r}
settingsDialog <- function(id) {
  modalDialog(
...
Because we're using a dynamic user interface, most of the work is going to happen on the server side. To keep things simple, here we'll going to expose a single control, the number of rows to display. However, we'll design the server function to be a little more general, returning a list of values so it's easier to extend in the future.

```{r}
settingsServer <- function(id, df, nrow = 6) {
  stopifnot(is.reactive(df))
  
  moduleServer(id, function(input, output, session) {
    output$controls <- renderUI({
      sliderInput(NS(id, "obs"), "Number of observations", min = 1, max = nrow(df()), value = nrow)
    })
    list(
      nrow = reactive(input$obs %||% nrow)
    )
  })
}
```

Note that we need to use `NS()` inside here, even though we're inside a server function. The automatic namespacing that `moduleServer()` performs only applies to `input`, `output`, and `session`; when generating user interface elements, we still need to call `NS()`.

Now we wrap it up into an app we can play around with:
...
settingsApp <- function(df) {
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$show_settings, {
      showModal(settingsDialog("settings"))
    })
    settings <- settingsServer("settings", df)
    
    output$table <- renderTable(head(df(), settings$nrow()))
  }
  shinyApp(ui, server)
}
```
...
2.  Rewrite `selectVarServer()` so that both `data` and `filter` are reactive. Pair it with a app function that lets the user pick the dataset with the `dataset` module, a function with an `inputSelect()` that lets the user filter for numeric, character, or factor variables.
...
6.  In `wizardServer()` verify that the namespacing has been set up correctly by using two or more wizards in a single add, and checking that you can navigate through each wizard independently.
