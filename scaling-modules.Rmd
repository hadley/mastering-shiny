...
Section \@ref(dynamic-wizard).

Most compliated piece of this module is the UI. We want each page (except the last) to have a link to the next page, and a link to the previous page (except the first). Arbitrary number of pages, so take a list. Also provide an optional component that lets the user of the module control what happens when the wizard is "done".
...
wizardUI <- function(id, pages, doneButton = NULL) {
  ...
  changePage <- function(i, label) {
    actionButton(NS(id, paste0("go_", i)), label)
  }
  
  # Wrap each page in a tabPanel with prev and next buttons
  for (i in seq_len(n)) {
    # First page only has next; last page only prev + done
    button1 <- if (i > 1) changePage(i - 1, "prev")
    button2 <- if (i < n) changePage(i + 1, "next") else doneButton
    wrapped[[i]] <- tabPanel(
      title = paste0("page_", i), 
      fluidRow(
        column(12, pages[[i]])
      ), 
      fluidRow(
...
For loop doesn't work - complicated to explain exactly why. But to do with late binding of `i`. Related to how `observeEvent` works; come back to later in book?

```{r}
wizardServer <- function(id, n) {
  moduleServer(id, function(input, output, session) {
    listen <- function(i) {
      observeEvent(input[[paste0("go_", i)]], {
        updateTabsetPanel(session, "wizard", selected = paste0("page_", i))
      })  
    }
    lapply(seq_len(n), listen)
  })
}
```
...
Let's use in a more realistic example where each page contains 
