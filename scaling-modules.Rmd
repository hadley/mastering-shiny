...
Our last and most complex module creates a helper for making wizards, a style of UI where the user is stepped through page-by-page. You saw an example of how to create a wizard interface using `tabsetPanel()` in Section \@ref(dynamic-wizard), and here we'll automate the whole process.

Most complicated piece of this module is the UI, and the most complicated part of the UI are the buttons. We want each page to link to both the previous and the next page. We'll first create a pair of functions to create those buttons:
...
Note the id of the button - we need to create unique identifiers so each button needs to include both the target and destination page. `"go_{from}_{to}"`

Then we write a function that creates a page of the wizard. This will contain a title (not shown, but used to identify the page for switching), the contents of the page (supplied by the user), and the two buttons. Not every page will have both buttons so I mark them as optional by supplying a default value of `NULL`.
...
Then we can put it all together. We take a list of pages and create the buttons for each page, then wrap up the user supplied page into a `tabPanel`, then combine all the panels into a `tabsetPanel`.
...
There are two little tricks here:

-   Firstly, the page navigation is a little trickier than I original described. The first page doesn't have a previous button, and the last page doesn't have a next button. Instead, for the last page, we allow the user to supply whatever control they like.

-   `tabsetPanel()` doesn't allow us to pass in a list of tabs, so we need to do a little `do.call()` magic to make it work. Hopefully this will be simplified in a future version of Shiny.

Now we've completed the UI side of the module, and we need to turn to the server. The only thing that the server needs to do is make the buttons work, i.e. we need to add an `observeEvent()` for every button to ensure that updates the tabset to the correct panel.

This poses a bit of a new challenge because we don't know exactly how many buttons there are, so we need to generate the `observeEvent()` calls dynamically. We'll come back to this idea in Chapter XYZ, but for now, just remember what we did in Section \@ref(multiple-controls): switch from `input$` to `input[[]]`.

```{r}
wizardServer <- function(id, n) {
  moduleServer(id, function(input, output, session) {
    listen <- function(from, to) {
      observeEvent(input[[paste0("go_", from, "_", to)]], {
        updateTabsetPanel(session, "wizard", selected = paste0("page_", to))
      })  
    }
    ids <- seq_len(n)
    lapply(ids[-1], function(i) listen(i, i - 1))
    lapply(ids[-n], function(i) listen(i, i + 1))
  })
}
```
...
Now lets use it in a slightly more realistic app that has inputs and outputs:
...
Note that final app includes a mix of namespaced and raw ids. This allows us as the module author to not have to worry about id names that user users. 
...
8.  In `radioExtraServer()`, return a list that contains both the value and whether or not it came from other.
