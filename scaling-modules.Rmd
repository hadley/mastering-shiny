...
In the last chapter we showed used functions to decompose parts of your Shiny app into independent pieces. Functions work well for code that is either completely in the server function or entirely within the UI. For code that spans both, i.e. whether the server code relies on specific structure in the UI, you'll need a new technique: **modules**.

At the simplest level, a module is UI and server function that live in their own file. The magic of modules comes because these functions are constructed in a special way in order to generate a namespace. So far, when writing an app, the "names" of the controls (the `id`s) are global: all parts of your server function can see all parts of your UI. Modules give you the ability to create controls that can only be seen from within the module. This is a called a **namespace** because it creates "spaces" of "names" that are isolated from the rest of the app.
...
The two levels of functions are important here. We'll come back to them later, but in short they help distinguish the argument to your module from the arguments to the server function. This is one of the bigger advantages of `moduleServer()` over the older `callModule()` style.
...
## Dynamic UI and modules

Using dynamic UI is relatively straightforward. The main challenge is figuring out whether you're in a UI context where id is explicit, or a server context where id is implicit. To demontrate this, I'm going to pull from a question in Shiny community: <https://community.rstudio.com/t/67898>.
