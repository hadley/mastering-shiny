...
It's important to understand that the reactive graph that controls the order in which your code is run. This is different from your usual imperative code where it's the order of lines in the source file that determines how the code is run. 

For example, here we could flip the order of the two lines:
...
This yields exactly the same reactive graph, so the order in which the code is run is exactly the same. However, it's confusing for humans to understand because the output refers to a reactive expression that's created later in the code. Don't do this! Instead, make sure that reactive expressions and outputs only refer to things defined early in the app, not later. This will make your code easier to understand.

Same is true for functions; can refer to function that hasn't been created

This is so important and a little surprising, so I'll say it again: the order in which reactive code is run is determined only by the reactive graph, not by the layout in the server function.

### Reactive expressions

There's one more important component that can appear in the reactive graph: the reactive expression, which allows you to extract out repeated computation. Reactive expressions have a flavour of both inputs and outputs:
...
There's no point in using one for this simple app, but I will, just to show you how it appears in the reactive graph:
...
We'll see an example of both abilities of reactive expression in the next section, when we explore a case study using a more realistic app.
...
Now that you understand the basic components of reactivity, and how to visualise them using the reactive graph, let's put all the ideas together and explore a more complex app. The goal of this app is to simulate random data from two normal distributions, display the data with a histogram, and perform a t-test. This is what the UI looks like:
...
ui <- fluidPage(
  fluidRow(
    column(3, 
      ...
      numericInput("mean", label = "µ", value = 0, step = 0.1),
      numericInput("sd", label = "σ", value = 0.5, min = 0.1, step = 0.1)
    ),
    column(9, plotOutput("hist"))
  )
)
...
draw_histogram <- function(x, binwidth = 0.1, xlim = c(-3, 3)) {
  df <- data.frame(x = x)
  ggplot(df, aes(x)) + 
...
server <- function(input, output, session) {
  x <- reactive(rnorm(input$n, input$mean, input$sd))
  output$hist <- renderPlot(draw_histogram(x()))
}
```
```{r, echo = FALSE, out.width = NULL}
app_screenshot(
...
server <- function(input, output, session) {
  params <- debounce(
    reactive(list(input$n, input$mean, input$sd)),
    250
  )
  x <- eventReactive(params(), {
    rnorm(input$n, input$mean, input$sd)
  })
  output$hist <- renderPlot(draw_histogram(x()))
}
