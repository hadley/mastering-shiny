...
This is the big idea in Shiny: you don't need to tell an output when to update, because Shiny automatically figures it out for you. How does it work? What exactly is going on in the body of the function? Let's thinkg about at the code inside the server function more precisely:
...
We've quickly skimmed over reactive expressions a couple of times, so you're hopefully getting a sense for what they might do. Now we'll to dive into more of the details, and show why they are so important when constructing real apps. 
...
Imagine I want to compare two simulated datasets with a plot and a hypothesis test. I've done a little experimentation and come up with the functions below: `histogram()` visualises the two distributions with a histogram, and `t_test()` uses a t-test to compares means and summarises the results with a string:
...
ui <- fluidPage(
  fluidRow(
    column(3, 
      ...
      numericInput("lambda2", label = "lambda1", value = 3),
...
```{r sim-button1-react, echo = FALSE, out.width = NULL, fig.cap="This reactive graph doesn't accomplish goal; we're added a dependency instead of replacing the existing dependencies."}
knitr::include_graphics("diagrams/basic-reactivity/timing-button.png", dpi = 300)
```

This yields the app in Figure \@ref(fig:sim-button) and reactive graph in Figure \@ref(fig:sim-button-react). This doesn't achieve our goal because it just introduces a new dependency: `x1()` and `x2()` will update when we click the simulate button, but they'll also continue to update when `lambda1`, `lambda2`, or `n` change. We want to _replace_ the existing dependencies, not add to them.
...
This concludes our whirlwind tour of reactivity. The next chapter will help you practice all the material you've seen so far by creating an bigger Shiny app to supports a data analysis.
