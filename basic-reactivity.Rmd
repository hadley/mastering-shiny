...
Inputs and outputs are directly connected to HTML components in your UI. You can also create your own reactive expressions that extract out repeated code within an app.
...
As you saw in Chapter \@ref(basic-app), reactive expressions allow you to extract out repeated computation. There's not much point in using one for this simple app, but I will, just to show you what the new reactive graph looks like:
...
Before you continue, I recommend running this app and trying it out.
...
## Reaching outside of shiny
 
So far, we've focussed on what's happening inside the app. But sometimes you need to reach outside of the app and cause side-effects to happen elsewhere in the world. This includes things like:
...
These don't affect your app, so you can't use `renderText()`, and you can't call them directly. And if you put directly inside your server function, you'll get an error about attempting to access a reactive value outside of a reactive context (and even if you didn't, it would only be run once.)

To solve this problem Shiny provides `observeEvent()`, it works similarly to `reactiveEvent()` except it acts like an ouput instead of a reactive expression. That means, rather than returning a meaningful value, the code inside must do something.

The place you'll use `observeEvent()` most often in your early days of shiny app is to help you debug:

```{r}
observeEvent(x1(), message("x1 has updated"))
observeEvent(x2(), message("x2 has updated"))
```

As you learn how reactivity works, you'll find you need this crutch less and less, but in the early days it can be extremely useful.
