...
This app works because the code doesn't _tell_ Shiny to create the string and send it to the browser, but instead, it informs Shiny _how it could_ create the string if it needs to. It's up to Shiny when (and even if!) the code should be run; it might be run as soon as the app launches, it might be quite a bit later, it might be many times, or it might never be run. This isn't to imply that Shiny is capricious, only that it's Shiny's responsibility, not yours, to decide when render logic is actually executed. So a better mental model is to think of giving Shiny recipes, not commands.

### Imperative vs declarative programming

This difference between commands and recipes is one of key difference between two important styles of programming:
...
* In __imperative__ programming, you issue a specific command and it's 
  ...
  visualise it, and save a report to disk.
...
With imperative code you say "Make me a sandwich". With declarative code you says "Ensure there is a sandwich in the refrigerator whenever I look inside of it". Imperative code is assertive; declarative code is passive-aggressive.

Most of the time, declarative programming feels tremendously freeing: you describe your overall goals, and the software figures out how to achieve them without further intervention. The downside is the occasional time where you know exactly what you want, but you can't figure out how to frame them in a way that the declarative system understands[^ggplot2-legends]. The goal of this chapter, and the later chapters that go into even more detail, is to decrease that feeling as much as possible. Generally, that involves building up you understanding of how Shiny works, so you can translate what you want into something that Shiny understands.

[^ggplot2-legends]: If you've ever struggled with getting a ggplot2 legend to look exactly the way you want, you've encountered this problem!

### Lazyness

One of the strengths of declarative programming in Shiny is it that it allows Shiny to be extremely lazy. It only ever does the minimal amount of work needed to update the output controls that you can currently see[^seeing]. This, however, comes with an important downside that you should be aware of. Can you spot what's wrong with this minor variation of the server function used above?
...
If you look closely, you'll notice that I've written `greetnig` instead of `greeting`. This will not generate an error in Shiny, but it won't do what you want. Because the `greetnig` output never exists, the code inside `renderText()` will never be run. If you're working on a shiny app, and you just can't figure out why your code never gets run, make sure that your UI and server functions are using the same names.

### The reactive graph

Shiny's lazyness has another important property. In most R code, you can understand the order of execution by reading the code from top to bottom. That doesn't work in Shiny, because code is run based on when needed. Instead, it's helpful to look at the __reactive graph__, which describes how inputs and outputs are connected.  The reactive graph for the app above is very simple:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/basic-reactivity/graph-1b.png", dpi = 300)
```

This graph tells you that whenever `name` is changed, `greeting` will need to be recomputed. 
...
There's one more important component in the reactive graph: reactive expressions. We'll come back to reactive expressions in detail very shortly, for now you can think of them as a tool for reducing duplication in your reactive code by introducing additional nodes into the reactive graph.

We don't need a reactive expression in our very simple app, but I'm going to add one anyway so you can see how it affects the graph:
...
Instead, this code yields the same reactive graph as above, so the order in which the code is run is exactly the same. Note, however, that organising your code like this is confusing for humans, and best avoided. Instead, make sure that reactive expressions and outputs only refer to things defined early in the app. This will make your code easier to understand.

This is very important and a little surprising, so I'll say it again: the order in which reactive code is run is determined only by the reactive graph, not by its layout in the server function.

## Reactive expressions

We've quickly skimmed of reactive expression a couple of times, so you're hopefully getting a sense for what they might do. Now we're going to dive into more of the details, and show why they are so important when constructing real apps.
...
In reactive programming, reactive expressions play a similar role to functions in ordinary analysis code: they are tools to reduce duplication.

Reactive expressions are important for two reasons:

* They help you create efficient Shiny apps that do the minimal amount of 
  recomputation when when their inputs change. This is the same basic idea as 
  tools like `make`, which help you define the dependencies between bigger 
  components of your workflow.

* They help to simplify the reactive graph making the flow of events easier
  to understand for humans.

To see these benefits we're going to explore a more complex app, but first, lets set the stage.

### The motivation

Imagine I'm simulating two datasets and I want to explore what's going on. I've created a couple of functions to help: one visualises the two distributions with a histogram, and the other compares their means with with a t-test:
...
Once I've simulated some data, I can use them to see what's happening:
...
Note that I've written functions here, a little earlier than I normally would, because I want to focus on the flow of reactivity below, rather than the details of the plot and hypothesis test. However, extracting code out into functions is a really important technique for Shiny apps: the more code you can put in regular functions outside of the app, the easier you will find it to test subcomponents of your app. This is good software engineer because it helps isolate concerns. Your functions outside of app can focus on the computation so that the code inside your app can focus on efficiently responding to user input.

### The app

Tweaking the parameters and re-running the code is going to get tedious fast, so I want to wrap the pieces into an Shiny app where I can interactively tweak the inputs. 
...
You've seen all these components before; we've just used more of them in a single app than in any previous example. Similarly, the server function doesn't introduce any new ideas, it just combines more pieces in one place.
...
Before you continue reading, I recommend opening the app and having a quick play: <https://hadley.shinyapps.io/basic-reactivity-cs/>. 
...
Let's draw the reactive graph of this app. 

Note that Shiny is smart enough to run code for an output when that only output needs to be updated. But it's not smart enough, however, to only selectively run pieces of code. For example, take this snippet from the server:
...
As a human looking at this code, you can tell that we only need to update `x1` when `n_1`, `mean_1`, or `sd_1` changes, and we only need to update `x2` when `n_2`, `mean_2`, or `sd_2` changes. Shiny, however, only looks at the output as a whole, so it will update both `x1` and `x2` every time one of `n_1`, `mean_1`, `sd_1`, `n_2`, `mean_2`, or `sd_2` changes. Another way of saying the same thing is that ordinary variables don't appear in the reactive graph.

This leads to the following reactive graph:
...
Note that you create a reactive expression using `reactive()`, assigning the results to a variable. To later use the expression in an output, call the variable like it's a function.
...
In Shiny, however, I think you should adopt the rule of one: whenever you copy and paste something _once_, you should consider extracting out the repeated code into a reactive expression. The rule is stricter for Shiny, because reactive expressions don't just make it easier for humans to understand the code, it also improves Shiny's ability to efficiently rerun code, so it can perform the minimal amount of work to update the outputs when an input changes. Later on, we'll see how to use shiny modules to extract out repeated code across multiple apps.

We'll come back to this idea in Chapter XYZ, when we discuss modules. Modules allow you to extract out repeated code and guarantee that it's isolated from other code in the app. Modules are an extremely useful and powerful technique for more complex apps.

### Reactive roadblocks {#reactive-roadblocks}

When you first start working with reactive code, you'll be tempted to draw on your existing tools for reducing duplication in code: creating new variables and writing functions. Unfortunately neither of these techniques work here.

If you try and use a variable, you might write code like this:
...
If you try this code, you'll get an error because you're attempting to access input values outside of a reactive context. Even if that did work, `x1` and `x2` will only be computed once, when the function runs, not every time one of the inputs was updated.
...
It has the following reactive graph:
...
```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/basic-reactivity/timing-click.png", dpi = 300)
```
...
server <- function(input, output, session) {
  params <- debounce(
    reactive(list(input$lambda1, input$lambda2, input$n)),
    250
  )

  x1 <- eventReactive(params(), {
    rpois(input$n, input$lambda1)
  })
  x2 <- eventReactive(params(), {
    rpois(input$n, input$lambda2)
  })
...
`observeEvent()` has two important arguments: `eventExpr` and `handlerExpr`. The first argument provides an expression (using reactive inputs and expressions) that determines when the second argument will be executed. For example, the following modification to `server()` means that everytime that `text` is updated, a message will be sent to the console:
...
It turns out that reactive outputs are actually a special type of observer, so they look the same in the reactive graph:
