...
Server functions take three parameters: `input`, `output`, and `session`[^basic-reactivity-3].
...
[^basic-reactivity-3]: For legacy reasons, `session` is optional, but you should always include it.
...
It's hard to show exactly how this works in a book, but if you run the app, and type in the name box, you'll notice that the greeting updates automatically as you type[^basic-reactivity-4]:

[^basic-reactivity-4]: If you're running the live app, notice that you have to type fairly slowly for the output to update one letter at a time.
...
With imperative code you say "Make me a sandwich"[^basic-reactivity-5].
...
[^basic-reactivity-5]: <https://xkcd.com/149/>
...
The downside is the occasional time where you know exactly what you want, but you can't figure out how to frame it in a way that the declarative system understands[^basic-reactivity-6].
...
[^basic-reactivity-6]: If you've ever struggled to get a ggplot2 legend to look exactly the way you want, you've encountered this problem!

### Laziness

One of the strengths of declarative programming in Shiny is that it allows apps to be extremely lazy.
A Shiny app will only ever do the minimal amount of work needed to update the output controls that you can currently see[^basic-reactivity-7].
...
[^basic-reactivity-7]: Yes, Shiny doesn't update the output if you can't see it in your browser!
...
Instead, make sure that reactive expressions and outputs only refer to things defined above, not below[^basic-reactivity-8].
This will make your code easier to understand.

[^basic-reactivity-8]: The technical term for this ordering is a "topological sort".
...
Because of this duality, some functions work with either reactive inputs or expressions, and some functions work with either reactive expressions or reactive outputs.
We'll use **producers** to refer to either reactive inputs or expressions, and **consumers** to refer to either reactive expressions or outputs.
...
I've done a little experimentation and come up with the functions below: `freqpoly()` visualises the two distributions with frequency polygons[^basic-reactivity-9], and `t_test()` uses a t-test to compare means and summarises the results with a string:

[^basic-reactivity-9]: If you haven't heard of a frequency polygon before, it's just a histogram that's drawn with a line, instead of bars, which makes it easier to compare multiple data sets on the same plot.
...
```{r ttest, echo = FALSE, out.width = "100%", fig.cap="A Shiny app that lets you compare two simulated distributions with a t-test and a frequency polygon", message = FALSE}
...
Reactive expressions automatically cache their results, and only update when their inputs change[^basic-reactivity-10].

[^basic-reactivity-10]: If you're familiar with memoisation, this is a similar idea.
...
```{r sim, echo = FALSE, out.width = "100%", fig.cap = "A simpler app that displays a frequency polygon of random numbers drawn from two Poisson distributions." , message = FALSE}
...
Imagine you wanted to reinforce the fact that this is for simulated data by constantly resimulating the data, so that you see an animation rather than a static plot[^basic-reactivity-11].
We can increase the frequency of updates with a new function: `reactiveTimer()`.

[^basic-reactivity-11]: The New York Times used this technique particularly effectively in their article discussing how to interpret the jobs report: <https://www.nytimes.com/2014/05/02/upshot/how-not-to-be-misled-by-the-jobs-report.html>
...
```{r sim-button, echo = FALSE, out.width = "100%", fig.cap = "App with action button.", message = FALSE}
...
These actions don't affect how your app looks, so you can't use an output and a `render` function.
...
This concludes our whirlwind tour of reactivity.
