...
Note the graphical conventions we used for the inputs and outputs. The `name` input naturally fits into the `greeting` output. We could draw them closely packed togther, as below, to emphasise the way that they fit together. We won't normally do that because it only works for the simplest of apps.

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/basic-reactivity/graph-1a.png", dpi = 300)
```

The reactive graph is a powerful tool for understand how your app works. As as your app gets more complicated, it's often useful to make a quick high-level sketch of the reactive graph to remind you how all the pieces fit together. Later on, in Chapter XYZ, you'll learn about Shiny's reactlog which will draw the graph for you, and animate how it changes as you run the app.

### Execution order

It's important to understand that the reactive graph that controls the order in which your code is run. This is different from most R code where the execution order is determined by the order of lines.
...
I have two functions that allow me to visualise two distributions with a histogram, and see if their means are different with a t-test:
...
I've extracted these out into functions. This is an important technique when working on code inside shiny apps - if there's a complex piece of programming that you want to test outside of shiny, pull it out into a separate function outside of the server function. That way you can figure it out independently of the challenges of reactivity.

I think it's good practice to do this as much as possible because it helps isolate concerns. Your functions outside of shiny do complex computations so that the code inside your app can focus on efficiently responding to user input.

SHOW EXAMPLE

And now I want to rapidly explore, but creating a shiny app.

### Basic app

The goal of this app is to simulate random data from two normal distributions, display the data with a histogram, and perform a t-test. This is what the UI looks like:
...
Let's draw the reactive graph of this app. Shiny is smart enough to run code for an output when that only output needs to be updated. But it's not smart enough, however, to only selectively run pieces of code. For example, take this snippet from the backend:
...
There's one other major flaw in the app: the histogram and t-test use independent random draws. This is rather misleading, as you'd expect them to be working on the same underlying data. 
...
Note that reactive expressions take inputs and produce outputs so they have a shape that combines features of both inputs and outputs. The shapes should help you remember how they fit together:
...
You should now be familiar with the basic ideas of reactivity: the server function that provides `input` and `output`, and the reactive graph that describes how code is executed. To finish up, we'll discuss two more advanced techniques that allow you to either increase or decrease how often a reactive expression is executed. Here I'll show how to use the basic techiques; in Chapter XYZ, we'll come back to the code that makes them tick.

To explore the basic idea, we're going to zoom in on a small portion of the whole, and just draw from a single distribution and display the results in a plot.
...
```
```{r, echo = FALSE, out.width = "33%", fig.show="hold", eval = FALSE}
app <- testApp(ui, server, deps = "ggplot2")
app_screenshot(app, "basic-reactivity/actionButton-1", width = 900)
app$setInputs(simulate = "click")
app_screenshot(app, "basic-reactivity/actionButton-2", width = 900)
app$setInputs(simulate = "click")
app_screenshot(app, "basic-reactivity/actionButton-3", width = 900)
```
