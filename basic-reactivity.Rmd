...
In the previous chapter, we talked about creating the user interfaces.
Now we'll move on to discuss the server side of Shiny, where you use R code at runtime to make your user interface come to life.
...
This chapter will provide a gentle introduction to reactive programming, teaching you the basics of the most basic reactive constructs you'll use in Shiny apps.
...
[^basic-reactivity-2]: The primary exception is where there's some work that can be shared across multiple users.
    ...
    We'll come back to that idea in Chapter XYZ.
...
We could draw them closely packed together, as below, to emphasise the way that they fit together; we won't normally do that because it only works for the simplest of apps.
...
Throughout this book we'll show you the reactive graph to help understand how the examples work, and later on, in Chapter XYZ, you'll learn how to use reactlog which will draw the graph for you.
...
2.  Can the reactive graph contain a cycle?
    Why/why not?
...
To emphasise this modularity the Figure \@ref(fig:ttest-module) draws boxes around the independent components.
...
Here I'll show how to use the basic techniques; in Chapter XYZ, we'll come back to their underlying implementations.
...
```{r sim-button, echo = FALSE, out.width = NULL, fig.cap = "App with action button.", message = FALSE}
