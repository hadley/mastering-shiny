...
As you can see, `server()` is function that you  define, and pass to `shinyApp()`, along with the UI. You'll never call a server function yourself; rather, Shiny invokes it whenever a new session begins, and almost of the reactive programming you'll do in Shiny will be directly in the server function[^exceptions]. 
...
To understand the server function, it's important to understand the idea of a __session__, which begins each time the Shiny app is loaded in a browser, either by different people, or by the same person opening multiple tabs. The server function encapsulates the connection to a single browser session, creating a private scope for that particular session. In other words, every variable you create inside the server function is only accessible to a single session. This ensures that when user A moves a slider, outputs update only for user A, not user B. 

Server functions always take three parameters: `input`, `output`, and `session`[^session-optional]. You'll never create these objects yourself; rather, they are created by Shiny when a session begins. Each combination of `input`, `output`, and `session` connects back to a specific session, and we'll use these objects to communicate back and forth with the browser. For the moment, we'll focus on `input` and `output`, and leave `session` for later chapters.
...
then you can access the value of the `count` input using `input$count`. It will initially contain the value `100`, and will be automatically updated as the user changes the value in the browser.
...
The reason for this restriction is that `input` is reflects what's happening in the browser; the browser is the "single source of truth". Being able to directly modify the `input` object would introduce a second source of truth, which could then be in conflict with the state of the browser. Later on, in Chapter XYZ, we'll show you how to work around this limitation with `updateNumericInput()` and friends.

One more important thing about `input`: it's a selective about who is allowed to read it. You must be in a __reactive context__ to be able to access it. We'll come back to that idea very shortly, but it's a fundamentally idea that allows outputs to automatically update when an input changes. For example, this server function will throw an error:
...
This is likely to be very surprising when you first start using Shiny. But this selectivity is extremely important as it allows shiny to automatically figure out how inputs and outputs are connected in order to automatically figure out what needs to be updated when ever an input changes.
