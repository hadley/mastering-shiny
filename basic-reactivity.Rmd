...
We've quickly skimmed over reactive expression a couple of times, so you're hopefully getting a sense for what they might do. Now we're going to dive into more of the details, and show why they are so important when constructing real apps.
...
Reactive expressions play a similar role to functions in imperative code: they are tools to reduce duplication.
...
If you were just exploring this data, you probably would've started with a bit more interactive exploration before writing functions. I've written functions here because I want to focus on the flow of reactivity below, rather than the details of the plot and hypothesis test. 

However, extracting regular imperative code out into functions is a really important technique for all Shiny apps: the more code you can put in regular functions outside of the app, the easier it will be to test subcomponents of your app. This is good software engineering because it helps isolate concerns: the functions outside of the app focus on the computation so that the code inside of the app can focus on reacting to user input.
...
You've seen all these components before; we've just used more of them in a single app than in any previous example. 

Similarly, the server function doesn't introduce any new ideas, it just combines a few more pieces in one place.
...
Shiny is smart enough to run code to produce an output only when the inputs it refers to change. It's not smart enough, however, to only selectively run pieces of code inside an output. For example, take this snippet from the server:
...
As a human reading this code you can tell that we only need to update `x1` when `n_1`, `mean_1`, or `sd_1` changes, and we only need to update `x2` when `n_2`, `mean_2`, or `sd_2` changes. Shiny, however, only looks at the output as a whole, so it will update both `x1` and `x2` every time one of `n_1`, `mean_1`, `sd_1`, `n_2`, `mean_2`, or `sd_2` changes.  This leads to the following reactive graph:
...
In the server function below we refactor the existing code to pull out the repeated code into two new reactive expressions, `x1` and `x2`, which simulate the data from the two distributions. You create a reactive expression using `reactive()`, assigning the results to a variable. To later use the expression in an output, call the variable like it's a function.
...
When you first start working with reactive code, you might try to draw on your existing tools for reducing duplication in code: creating new variables and writing functions. Unfortunately neither of these techniques works in a reactive environment.
...
If you try this code, you'll get an error because you're attempting to access input values outside of a reactive context. But even if that did work, `x1` and `x2` would only be computed once, when the session begins, not every time one of the inputs was updated.
...
To explore the basic ideas, I'm going to simplify my simulation app. I'll use a distribution with only one parameter, and force both samples to share the same `n`. I'll also remove the plot controls. This yields:
...
Imagine you wanted to reinforce the fact that this is for simulated data by constantly resimulating the data, so that you see an animation rather than a static plot[^nytimes].
...
We can do that simply by using `reactiveTimer()`. `reactiveTimer()` is a reactive expression that has a dependency on a hidden input: the current time. You can use a `reactiveTimer()` when you want a reactive expression to invalidate itself more often than it otherwise would. The following code uses a time out of 500 ms so that the plot will update twice a second. This is fast enough to remind you that you're looking at a simulation, without dizzying you with rapid changes.
...
In the above scenario, think about what would happen if we had to recompute everything every 0.5s, but the simulation took 1s to run. Shiny would have more and more to do, and would never be able to catch up. The same problem can happen if someone is rapidly clicking buttons in your app and the computation you are doing is relatively expensive. It's possible to create a big backlog of work for shiny, and while it's working on the backlog, it can't respond to any new events.
...
To use the action button we need to learn a new tool. To see why, lets first tackle the problem using the same approach as above. We refer to `simulate` without using it's value in order to take a reactive dependency on it.
...
To solve this problem we need a new tool; something that lets use the values of an input without taking a reactive dependency on it. We need `eventReactive()`, which has two argument: the first argument specifies what to take a dependency on, and the second argument runs the code that yields the result. That allows this app to only compute `x1()` and `x2()` when `simulate` is clicked:
...
Note that, as desired, `lambda1`, `lambda2`, and `n` are not connected to `x1` and `x2`: changing their values does not cause any code to be recomputed. I have drawn their arrows in very pale grey just to remind you that `x1` and `x2` are using those values, without taking a reactive dependency on them.
...
The key difference between `observeEvent()` and `eventReactive(oh ok)` is that observe event creates an observer which you don't assign to a variable, and you can't use in other reactive expressions or outputs. It turns out that reactive outputs are actually a special type of observer, so they look the same in the reactive graph:

```{r, echo = FALSE, out.width = NULL}
knitr::include_graphics("diagrams/basic-reactivity/graph-3.png", dpi = 300)
```

Outputs are also used for their side-effects: updating the HTML in the users browser. 
