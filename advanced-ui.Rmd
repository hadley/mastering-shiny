...
deps is a list containg 4 dependencies:
  ...
  - [fontawesome](https://fontawesome.com) handles icons
...
Now, you may imagine the possibilities are almost unlimited! In exercise 1, we see 
how to integrate a Bootstrap 4 box in Shiny.

### Exercises

1.We explained above that Shinydashboard is built on top of adminLTE2, a specific admin template. There is actually more recent version in development containing boxes with a better design (Bootstrap 4). Let's try to update our old Shinydashboard box component. To help you in the process here are some indications:
- first include Bootstrap 4 CSS dependencies (in this example, we don't need the javascript code).
- Explore the [Bootstrap 4 card](https://getbootstrap.com/docs/4.3/components/card/) page to include a new card component and convert the correspond HTML code to R with htmltools utilities like
`withTags`, `tags`...
- Do not forget to attach the Bootstrap 4 dependency to you new tag
- Include your brand new box in a shiny app
...
bs4_card_dep <- function() {
  htmltools::htmlDependency(
    name = "bs4_card",
    version = "1.0",
    src = c(href = "https://maxcdn.bootstrapcdn.com/bootstrap/4.3.1/css/"),
    stylesheet = card_css
  )
}
```
...
function. Notice below that the box CSS class from adminLTE2 is now card in the version 3.
...
my_card <- function(...) {
  cardTag <- htmltools::withTags(
    div(
      ...
      div(class = "card-header bg-transparent border-success", "Header"),
      div(
