...
In most case, you can use `list()` instead of `tagList()`; there's only a few places where the difference is important. One place where using a list directly is when generating HTML programmatically. For example, the snippet below uses `lapply` to simplify the previous example:
