...
In this chapter, you'll learn how writing functions can help reduce complexity by extract unrelated concerns in to individual functions. This tends to have slightly different flavours for UI and server components:
...
Functions have another important role in Shiny apps: they allow you to spread the logic in your app across multiple files. While you certainly can have one giant `app.R` file, containing a massive `ui` specification and large `server` function, it's much easier to manage when spread across multiple files. When done in the right way, this makes it easy to understand the big picture so you only have to dive into the details when necessary.
...
If you've made an R package before, you might notice that Shiny uses the same convention for storing files containing functions. And indeed, if you're making a complicated app, particularly in conjunction with others, there are substantial advantages to making a full fledged package. If you want to do this, I recommend the reading the ["Engineering Shiny"](https://engineering-shiny.org) book and using the accompanying [golem](https://thinkr-open.github.io/golem/) package. We'll touch on packages again when we talk more about testing because packages provide useful infrastructure for testing.
