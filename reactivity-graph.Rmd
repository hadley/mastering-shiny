...
If it's been a while since you looked at Chapter \@ref(basic-reactivity), I highly recommend that you re-familiarise yourself it with before continuing.
...
The invalidation phase starts at the changed input/value we'll fill with grey, our usual colour for invalidation, as in Figure \@ref(fig:graph-input-changes).
...
But this is an key part of Shiny's reactive programming model: though these particular arrows *were* important, they are now out of date.
...
This would have no impact on the output of normal R code, but it makes a difference here because the reactive dependency is established when you read exact a value from the `input` with `$`.s
...
There's a surprise in this screenshot compared to the graph we drew earlier --- there are three additional reactive inputs (`clientData$output_x_height`, `clientData$output_x_width`, and `clientData$pixelratio`) that don't appear in the source code for our graph.
These exist because the plot must be redrawn whenever the output changes size.
...
In particular, you've learned for the first time about the invalidation phase, which doesn't immediately cause recomputation, but instead makes reactive consumers as invalid, so that they will be recomputed when need.
