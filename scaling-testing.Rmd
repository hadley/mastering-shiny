...
This chapter uses features in the upcoming release of testthat 3.0.0 and shinytest 1.5.0, so if you're reading the book now you'll need to install their development versions from GitHub with `remotes::install_github(c("r-lib/testhat", "rstudio/shinytest"))`.
...
The easiest part of your app is test is the part that has the least to do with Shiny: the functions extracted out of your UI and server code as described in Chapter \@ref(scaling-functions).
...
Despite the fact that we're passing a complete app to `testServer()`, we are only testing the server function; the `ui` component of the app is completely ignored.
...
This time, the first argument is the module server, and then we supply the arguments to that function in a new argument called `args`.
This takes a list of arguments to the module server, always omitting the `id` argument, since that's automatically filled in by `testServer()`.
...
This might not seem important because we haven't talked at JavasSript in this book, but there are a number of important Shiny functions that use it behind the scenes:
...
And then as before, you can turn into a test by wrapping in `test_that()` and calling expectations:
