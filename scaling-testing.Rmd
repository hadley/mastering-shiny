...
(Note that we're not testing an app, we're testing just the server function, and `ui` is not used by `testServer()`.)
...
Unlike a real Shiny app, all inputs start as `NULL`. That's because this is a pure server side simulation; while we give it that app object that contains both the UI and server, it only uses the server function. We'll talk more about this limitation and how to work around shortly.
