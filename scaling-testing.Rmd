...
We'll perform automated testing with the [testthat](http://testthat.r-lib.org/) package, the a popular testing package[^scaling-testing-1]. A testthat test looks like this:

[^scaling-testing-1]: It's used by over 4,700 packages on CRAN.
...
We'll come back to the details very soon, but note that a test starts by declaring what it test (\"`as.vector()` strips names) then uses regular R code to generate some test data. The test data is then compared to the expected result using a **expectation**, a function that starts with `expect_`. Here we verify that the ouput of `as.vector(x)` equals `c(1, 2)`.
...
-   Then we'll test parts of Shiny that use **javascript** (including the `update*()` family of functions) by running the app in a web browser. This is a higher fidelity simulation because it runs a real browser in the background, but the downside is that tests are slower to run, and you can no longer peek at reactive values.

-   Finally, we'll test **app visuals** by saving screenshots of selected elements. This is necessary for testing app layout and CSS, plots, and HTML widgets, but screenshots can change for unrelated reasons and a human intervention is required to confirm whether each change is ok or not.

These levels of testing form a natural hierarch because each technique provides a fuller simulation of the user experience of an app. The downside of the better simulations is that each level gets progressively slower because it has to do more, and more fragile because it's harder to verify changes automatically. So when writing tests, you should always strive to test at the lowest level so your tests are as fast and reliable as possible. Over time this will also influence the way you write code: knowing what sort of code is easiest to test tends to naturally push you towards clearer designs.

Interleaved between these levels of testing, I'll also provide advice about testing workflow and more general testing philosophy.
...
This chapter uses features in the upcoming release of testthat 3.0.0, so if you're reading the book now you'll need to install the development version of testthat from GitHub: `remotes::install_github("r-lib/testhat")`.
...
-   As I develop more tests, I run all of the tests for the current file[^scaling-testing-2] with `devtools::test_file()`. Because I do this so often, I have a special keyboard shortcut set up to make it as easy as possible. I'll show you how to set that up yourself very shortly.
...
Let's start with a simple module. This module uses three outputs to dispay a brief summary of a variable:
