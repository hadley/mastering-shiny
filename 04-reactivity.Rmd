...
"See, the value of `x` just changed from 0 to 10!"
...
So far so good--`temp_celsius` is `10`, and `is_freezing` is `TRUE`. But once we change `temp_celsius`:
...
`temp_celsius` has changed, but unless you take special action, any derived values like `is_freezing` remain unchanged and are now out of date.
