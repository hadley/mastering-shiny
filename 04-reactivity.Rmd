...
The biggest drawback of reactive programming is that it is not intuitive at first. Even experienced R users can have trouble getting their heads around it as they get started with Shiny, and those with deep experience in software engineering may dismiss it as too "magical". But once we've formed an accurate mental model, you'll see that there is nothing to fear.
...
In one sense, all programmers are used to "values that change over time"--they're called "variables", right? Variables in R represent values and they can change over time. But they're not particularly designed to help you when their values change.
...
Reactive programming is a way of structuring your R code so that relationships between changing values (like `temp_celsius`) and the derived expressions that rely on them (like `is_freezing`) are automatically detected and maintained.
...
If a read of a regular variable is asking "What's the value of `x`?", reading a reactive value is asking "What's the value of `input$x`? **And also notify me the next time `input$x` changes, please!**"

Now let's shift gears and talk about who exactly the reader is in this scenario, and what will happen when they're notified of changes in `input$x`.
...
Once the tallying is done, we'll look at this data in a couple of ways. We'll zoom in on the top few most popular values (e.g. the most popular packages):

```{r echo=FALSE, message=FALSE, fig.height=4, fig.width=5.5}
print(readRDS("images/04-reactivity/top_packages.rds"))
```

And we'll also visualize all the values to get a sense of the "long tail", to show how quickly the downloads decrease as we move down the list of packages.

```{r echo=FALSE, message=FALSE, fig.height=4, fig.width=5.5}
print(readRDS("images/04-reactivity/all_downloads.rds"))
```
...
Again, the steps in this script are:
