...
That's because normally in Shiny you work with a special `reactiveValues()` object:
...
A `reactiveValues()` works like a regular list.
...
This might look familiar --- ever app you've written works with `inputs` in this way.
`inputs` is a special type of `reactiveValues()` object that is read only.
You're not allowed to touch the values because Shiny automatically updates them based on the users action in the browser.
...
Occasionally, you want to access the value of a reactive producer without taking a dependency on it.

`observe()` is the building block that powers the `observeEvent()` function that you learned about in Section \@ref(observers).
`observeEvent(event, action)` is basically equivalent to:
...
In Chapter \@ref(reactivity-components) we'll see how you can use reactive values, `observe()`, and `isolate()` to manage changes to state over time.
For example, maybe you want to display the number of times a button is pushed, or (more usually) combing the results of the current brushing gesture with the points that are already selected.
