...
How does this connect to real Shiny apps?
You don't normally have to call `reactiveVal()` and instead work with the `inputs` argument.
`inputs` is a minor variation of `reactiveVal()` called `reactiveValues()` --- instead of managing a single reactive value, it manages a list of reactive values.
...
But if you use a reactive expression, it will automatically update when the values change:
