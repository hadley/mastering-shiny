...
[^password]: All `passwordInput()` does is hide what the user is typing, so that someone looking over their should can't read it. It's up to you to make sure that any password are not accidentally exposes, so we don't recommend using passwords unless you have had some training in secure programming.
