...
<!-- MCR: The screenshot is quite small, making it hard to tell the icons apart on the rendered book. -->
...
<!-- MCR: I think it's worth mentioning varSelectInput() so people don't try to re-engineer it. For example...
...
```
```{r, echo = FALSE, out.width = NULL, eval=FALSE}
# Should be eval=TRUE if included
ui_screenshot(ui, "basic-ui/var-select")
```

Or if you think it's too complicated for this stage, a sentence like "We will see an example of this in Chapter XYZ." could be added instead of the example. This example isn't complicated, but the server side of this input is more so... -->
...
MCR: Couple more exercises.

1.  Create a slider input to select values between 0 and 100 where the interval between each selctable value on the slider is 5. Then, add animation to the input widget so when the user presses play the input widget scrolls through automatically.

1.  Using the following numeric input box the user can enter any value between 0 and 1000. What is the purpose of the step argument in this widget?
...
<!-- MCR: How about quoting c? So it reads, 'returns `"c"`' -->
...
MCR: I think this point could be more more clear by adding the following to the code chunk above.
...
MCR: Do you want to go into the whole server/client side tables thing and how DT package allows for that? If so, a rephrasing of the note from the dataTableOutput() docs might be good enough to make that point. I don't think it's necessary, but people do ask about the DT package since it shows up in many examples in the wild.
...
<!-- MCR: The following language could use some (lots of) love, but saying something about curly braces here makes sense since we have the options defined outside of them.

Note that in the `renderDataTable()` function above we used curly braces around the expression that is being rendered. When the expression only spans one line of code, these curly braces are not required. However the code needed to develop the expression for the object to be rendered could also span multiple lines of code. Curly braces help define the complete expression, even if it spans multiple lines of code.
...
1.  Move the sidebar to the right in sidebarLayout()

2.  A sample app where some commas are missing betweem layout elements

3.  Change the theme of an app from an earlier exercise in the book
