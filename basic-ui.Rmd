...
`fileInput()` special handling on the server side, and is discussed in detail in Chapter \@ref(action-transfer).
...
`sidebarLayout()`, along with `titlePanel()`, `sidebarLayout()`, `sidebarPanel()`, and `mainPanel()`,  makes it easy to create a two-column layout with inputs on the left and outputs on the right. The basic code is shown below; it generates the structure shown in Figure \@ref(fig:ui-sidebar).
