...
Outputs have few arguments because, by and large, they just create recepticles for output. There are three main types of output, corresponding to the three things you usually include in a data report: text, tables, and plots. We'll show you three main types output functions below, along with the `render` functions that they're usually coupled with.
...
`plotOutput()` has a number of arguments like `click`, `dblclick`, and `hover`. If you pass these a string, `click = "plot_click"`, they'll create a reactive output that you can use to handle user events. We'll come back to that idea in Chapter XYZ.
