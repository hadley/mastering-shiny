...
There are two cases where a reactive input might be invalidated without the user doing anything:

-   If you've called, `update*(value = )` sends a message to the browser to change the value.

-   If you have a `reactiveVal()` (or `reactiveVals()`) that you've modified.
...
It emits a special type of error[^reactivity-tracking-3] that Shiny swallows instead of displaying in the outputs.
