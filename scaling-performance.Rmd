...
[^scaling-performance-2]: Again, this depends on exactly how your app is deployed, but it's typically a prominently labelled setting in the tool that runs your Shiny apps.
...
Unfortunately, R is single-threaded, which means that can't do multiple things at the same.
This is fine if all of the meals are fast to cook, but if someone requests a 24-hour sous vide pork belly, all customers will have to wait 24 hours before the chef can start on their meal.
...
You can see that the output looks very similar.
