...
In this chapter, we'll form a mental model of reactive programming. Shiny wouldn't exist without reactive programming, as is by far the most elegant, robust, and productive way of of creating dynamic user interfaces.

Before reactive programming entered the mainstream, most people use traditional OO programming to create user interfaces. This requires an inordinate amount of effort and complexity because you must explicitly manage the relationships between different pieces of UI, and carefully update the cascading sets of variables that depend on the UI and on each other. This makes it very hard to create code that's fast, correct, and easy to write and maintain. 

In contrast, with just reactive values (inputs), reactive expressions, and observers (outputs), we have a rich vocabulary for expressing calculations and actions that forms the foundation for a better way to do UI programming. As long as we can break up our logic into distinct-but-related calculations, actions, and outputs, Shiny's reactive framework will automatically handle the tedious and error-prone task of deciding what pieces need to be updated when.
...
Reactive programming elegantly solves both problems by combining features of each of the solutions above. Now we can show you some real Shiny code. We're going to do this using a special Shiny mode, enabling the reactivity in the console so you that you can experiment with it directly, outside of an app. This mode is isn't enabled by default because it makes a certain class of bug harder to spot in an app, and it's primary benefit is to help you understand reactivity.
...
[^mutable-cell]: If you've programmed in languages like rust or clojure this might look familiar: this is a mutable cell.
...
Notice that the conversion only happens if we request the value of `temp_f()`, and the computation happens only once. Shiny strives to be as lazy as possible!
