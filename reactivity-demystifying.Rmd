...
vals <- reactiveValues()
vals$x <- 10
observe(message("`x` is ", vals$x))

vals$x <- 20

vals$x <- 30

consoleReactive(FALSE)
```

```{r}
library(rlang)
`%<R-%` <- function(name, value = NULL, env = parent.frame()) {
  name <- as.character(ensym(name))
  rv <- shiny::reactiveVal(value, name)
  
  if (env_has(env, name)) {
    env_unbind(env, name)
  }
  env_bind_active(env, !!name := rv)
...
is_freezing <- reactive(temp_celsius <= 0)
is_freezing()
