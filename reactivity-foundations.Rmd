...
I personally prefer `reactiveVal()` because the syntax makes it's more clear that something weird is going on; in this book I use `reactiveValues()` because the syntax is easier to understand at a glance.
...
shinyApp(ui, server)
#> Error: Can't modify read-only reactive value 'name'.
```
...
It is useful to think of `observe()` not as doing something, but as creating something (which then takes action as needed).
...
`isolate()` also works inside `reactive()`, but tends to be less use because `eventReactive()` takes care of most isolation needs for reactive expressions.
