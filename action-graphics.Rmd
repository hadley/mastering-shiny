...
[^action-graphics-1]: Shiny didn't support touch events when I wrote this chapter, but it might by the time you read this.
...
Just make sure to explain to the user what they can do: one downside of using mouse events to interact with an app is that they're not immediately discoverable[^action-graphics-2].

[^action-graphics-2]: As a general rule, adding explanatory text suggests that your interface is too complex, so is best avoided, where possible.
...
Unfortunately this requires an advanced reactivity technique that you have yet learned about: `reactiveVal()`.
...
To make this work, I initialise the `reactiveVal()` to a vector of `FALSE`s, then use `brushedPoints()` and `ifelse()` toggle their values: if they were previously excluded they'll be included; if they were previously included, they'll be excluded.
...
For example, if you're using a faceted plot in ggplot2, you might use it to increase the size of the plot to keep the individual facet sizes roughly the same[^action-graphics-3].

[^action-graphics-3]: Unfortunately there's no easy way to keep them exactly the same because it's currently not possible to find out the size of the fixed elements around the borders of the plot.

## Cached plots

If you have an app with complicated plots that take a while to draw, you can get some major performance improvement with plot caching.
...
You'll notice one important difference between `renderPlot()` and `renderCachedPlot()`: a cached plot also needs a `cacheKeyExpr`, an expression that uniquely identifys each plot.
...
Here we'll focus on the big picture; for the full details you can refer to the [Shiny website](https://shiny.rstudio.com/articles/plot-caching.html).
...
-   `cache = diskCache(...)`: shares across multiple users, multiple processes, and app restarts.
...
It's also possible to store in a database, or write your own backend.
...
For example, you might have a directory of photographs that you want show to the user.
