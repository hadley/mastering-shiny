...
1.  Use the `browser()` technique described in Section \@ref(clicking) to verify what `brushedPoints()` returns.
    How does `allRows = TRUE` change the output?

2.  Make a plot with click handler that shows all the data returned in the input.

3.  Make a plot with click, dblclick, hover, and brush output handlers and nicely display the current selection in the sidebar.
    Plot the plot in the main panel.

4.  Compute the limits of the distance scale using the size of the plot.
...
To do this, supply zero-argument functions to the `width` and `height` arguments of `renderPlot()` --- we now need to define these in the server, not the UI, because they can change.
...
-   The sizing policy, which ensures that plot is cached even when the sizes are a little different.
...
-   `cache = "session"`: the cache lifetime will be tied to a single user.
    It will be created when the session starts (i.e. when the app is first opened), and deleted when the session ends (i.e. when the app is closed.
...
```{r puppies, echo = FALSE, message = FALSE, fig.cap = "An app that displays cute pictures of puppies using `renderImage()`.", out.width = "50%"}
