...
Another way to use `nearPoints()` is to compute the distance between each point and the event. `nearPoints(allRows = TRUE, addDist = TRUE)` to return all rows with distance; this allows you to rely on the distance calculations done by Shiny, but you can use the information in other ways. For example, the following plot 
