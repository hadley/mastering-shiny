...
To demonstrate the basic approach, I'll show you a little problem I encountered when writing Section \@ref(hierarchical-select). The initial goal is pretty simple: I have a dataset of sales, and I want to filter it by territory. Here's what the data looks like:
...
I thought this seemed like a simple enough problem that I could just write the app, so I quickly wrote the following very simple app. It's an eight line app, I thought, what could possibly go wrong!
...
Well, when I opened the app up I got a _lot_ of missing values, no matter what territory I selected. So I quickly re-ran the code in R:

```{r}
head(sales[sales$TERRITORY == "EMEA", ])
```
Ooops! I'd forgotten that `territory` contained a bunch of missing values and `NA == NA` returns `NA`. One way to fix this problem is to switch from `==` to `%in%` because`NA %in% NA` is returns `TRUE`. (There are a few other ways you could've fixed this problem. `subset()` automatically removes `NA`s, as does `dplyr::filter()`. You could also wrap it in `which()`, which returns a integer vector giving the location of `TRUE` values and as a side-effect drops any `NA`s.)

```{r}
head(sales[sales$TERRITORY %in% "EMEA", ])
```

This fixed most of the problems, but now when I selected `NA` in the dropdown, I didn't get any output. This was strange because it worked on the console:

```{r}
head(sales[sales$TERRITORY %in% NA, ])
```
So I needed to perform an experiment to figure out why the code run in Shiny was different to the code I was running in the console. It seems like that the most likely place to for there to be a problem was on in the reactive that does the subsetting. So I added a `browser()` statement there. That also required adding `{}`. 

```{r}
server <- function(input, output, session) {
  selected <- reactive({
    browser()
    sales[sales$TERRITORY %in% input$territory, ]
  })
  output$selected <- renderTable(head(selected(), 10))
}
```

Now when my app ran, I was immediately dumped into an interactive console.  Luckily the selected value was `NA` by default so I could immediately dive in. Otherwise, I would've needed to press `c` to coninue running the app, select the value in the dropdown that was causing me problems and go back to the interactive debugger.

I first ran `sales[sales$TERRITORY %in% input$territory, ]` to confirm that I could see the same problem in the console that I was seeing in the app. If I didn't see the problem, I must be debugging in the wrong place, and I'd have to come up with a new hypothesis. This is an important part of debugging: you need to carefully check everything that you think you know.

But it wasn't selecting any rows, so I could proceed to the next step. I first check that `sales` looked like what I expected.the app was is simple that it seems unlikely for `sales` to be corrupted, but obviously something was wrong with my expectations so best to double. The dataset looked ok, so it must be something to do with the input.

So I then looked at `input$territory`:
...
I stared at this for a while because it looked ok too. Then it occurred to me! I was expecting it to be `NA`, but it's actually `"NA"`!  That gave the tools needed to fix the problem. 

Now I could recrate the problem outside of the Shiny app:

```{r}
sales[sales$TERRITORY %in% "NA", ]
```
...
The problem went away so I was done!

It also felt to me like Shiny should be taking care of this situation, so I also filed a bug report: <https://github.com/rstudio/shiny/issues/2884>.
