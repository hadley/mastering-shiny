...
-   If you want to start a new project[^action-workflow-2], go to the File menu, select "New Project" then select "Shiny Web Aplication":
...
Another way to increase your iteration speed still further is to turn autoreload on (`options(shiny.autoreload = TRUE)`) and then run the app in a background job, as described in <https://github.com/sol-eng/background-jobs/tree/master/shiny-job>.
...
There are two other options that you can choose from the Run App drop down:
...
Every error is accompanied by a traceback, or call stack, which literally traces back through the sequence of calls that lead to the error.
...
You might already be familiar with `traceback()`.
You can run this function interactively to see the sequence of calls that lead to an error, *after* the error has occured.
Unfortunately, you can't use this function in Shiny because you can't interactively run code while an app is running.
...
-   The first few calls start the app:
    ...
    Here the file is `source()`d, then `print.shiny.appobj()` calls `runApp()` to start the app.
...
If you're using RStudio, the toolbar in Figure \@ref(fig:debug-toolbar) will appear when you're in the debugger.
...
As well as stepping through the code line-by-line using these tools, you'll also run a *bunch* of interactive code to understand why you're seeing the error you see.
...
Ooops!
I'd forgotten that `territory` contained a bunch of missing values and `NA == NA` returns `NA`.
I quickly brainstormed a few possible solutions.
I could:
...
I decided to use the first form[^action-workflow-3] because I think it's the most explicit, and has the pleasant side-effect of reducing the duplication in my code.
I then double-checked that this had the result I expected:
...
And then I remembered, of course this won't work, because of the `NA` problem I already thought about --- I had forgotten that the `NA` might also appear on the right-hand side.
Fortunately, one of the techniques that I'd already brainstormed came to the rescue:
...
At this point, I figured I'd done everything I could on the console, and I needed to perform an experiment to figure out why the code inside of Shiny was behaving differently to my expectations.
...
Luckily the default value in the select input was `NA` so I could immediately dive in to the problem.
If that wasn't the case, I would have used `c` to continue execution, and then interacted with the app to get it into the state I needed.

Then, to check I'd put the `browser()` in the right place, I ran `subset(sales, TERRITORY %in% input$territory)`.
Indeed, it returned an empty data frame, so I could proceed to the next step.

I then checked that the `sales` dataset looked like I expected.
I didn't really expect it to be corrupted, since nothing in the app touches it, but best to be certain before continuing.
`sales` looked ok, so it must be something to do with the selection condition.
...
This is typically much easier if it's your own code, because you already understand what all the parts do.
...
This isn't some make work exericse I use to fob off people I don't want to help: it's exactly where I start!
