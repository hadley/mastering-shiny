...
[^embracing]: There's another form of indirection that happens when you're writing functions which is solved using `{{ x }}`, called embracing. You can learn more about that in  [_Programming with dplyr_](http://dplyr.tidyverse.org/dev/articles/programming.html).
...
Let's take a look at a more complicated example where we allow the user to create a plot by selecting the variables to appear on the `x` and `y` axes:
...
To refer to variables indirectly use `any_of()` or `all_of()`[^one-of]: both expect a character vector env-variable containing data-variable names. The only difference is what happens if you supply a variable name that doesn't exist in the input: `all_of()` will throw an error, while `any_of()` will silently ignore it.
