...
To learn more about using tidy evaluation in a function or package, you'll need to turn to other resources like [*Using ggplot2 in packages*](http://ggplot2.tidyverse.org/dev/articles/ggplot2-in-packages.html) or [*Programming with dplyr*](http://dplyr.tidyverse.org/articles/programming.html).
...
In most (but not all[^action-tidy-1]) base R functions you must to refer to data-variable using `$`, which tends to create code that repeats the name of the data frame multiple times:
...
When using Shiny[^action-tidy-2], indirection mostly frequently occurs when you have the name of a data-variable stored in an env-variable that comes with a reactive input.
Learning how to work with this form of indirection is the key to effectively using Shiny with the tidyverse.

[^action-tidy-2]: There's another form of indirection that happens when you're writing functions which is solved using `{{ x }}`, called embracing.
    You can learn more about that in [*Programming with dplyr*](http://dplyr.tidyverse.org/dev/articles/programming.html).

To make the problem concrete, take the following code which uses data-masking to refer to a data-variable (`carat`) and env-variable (`min_carat`) in the same way[^action-tidy-3]:

[^action-tidy-3]: Note that an expression like `carat > min_carat` has to look for three things: `carat`, `min_carat`, and `>`.
...
To get started we can rewrite our previous `filter()` call to use `.data` to make it clear that `carat` is a data-variable[^action-tidy-4]:

[^action-tidy-4]: `.data` is paired with `.env`, which isn't usually needed, but we'll come back to it later in Section \@ref(user-data).
...
The UI will have a input to select the variable, an input for the minimum value, and a output to show the data, and the server function will wrap the `filter()` call in a reactive and render the first few rows.
...
The following app uses a `switch()` statement to genereate a reactive geom that is later added to the plot.
...
If you experiment with this code, you'll find that it appears to work just fine for vast majority of data frames.
...
We can resolve the problem by using `.env`[^action-tidy-5] to tell `filter()` only look for `min` in the env-variables:

[^action-tidy-5]: You might wonder if the same problem applies to variables called `.data` and `.env`.
...
To refer to variables indirectly use `any_of()` or `all_of()`[^action-tidy-6]: both expect a character vector env-variable containing the names of data-variables.
...
[^action-tidy-6]: In older versions of tidyselect and dplyr, you'll need to use `one_of()`.
