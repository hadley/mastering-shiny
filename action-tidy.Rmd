...
The blurring of data-variables and env-variables makes data analysis easier at the cost of making indirection harder. In Shiny[^embracing], indirection happens when you have the name of data-variable stored in an env-variable, and is the key in use data-masking with Shiny apps. 
...
Let's wrap this up into a simple Shiny app. The UI will have a input to select the variable, an input for the minimum value, and a output to show the data, and the server function will wrap the `filter()` call in a reactive, render the first few rows.

```{r}
ui <- fluidPage(
  selectInput("var", "Variable", choices = names(diamonds)),
  numericInput("min", "Minimum")
  tableOutput("output")
)
...
Note that if we didn't use `.data`, as below, the app will not work correct, but also won't generate an error message. That's because `input$var` will be a string like `"carat"` and `"carat" > 0` is valid R expression that evaluates to `TRUE`, selecting all rows.
...
Before we go, it's worth a brief comment about `paste()` + `parse()` + `eval()`. If you have no idea what this combination it, you can skip this section, but if you have used it, I'd like to pass on a small note of caution.
