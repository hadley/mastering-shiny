...
Let's begin with this call to `filter()` which uses a data-variable (`carat`) and an env-variable (`min_carat`):
...
```{r user-supplied, fig.cap = demo$caption("An app that filter users supplied data, with a surprising failure mode"), echo = FALSE, message = FALSE}
...
1.  Why does `filter(diamonds, "carat" > 1)` return all rows?
