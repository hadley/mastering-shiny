...
Take the following app: it allows the user to upload a tsv file, then select a variable, and filter by it. It will work for the vast majority of inputs you might try it with:
...
However, there is a subtle problem with the use of filter. Let's zoom in on that code so we can play around.
...
If you experiment, you'll find that this works perfectly for the vast majority of data frames. However, there's one big problem: if the data frame contains a variable called `input`:
...
We can resolve this problem by telling `filter()` to never look in the data frame for `input$var`.
...
(Alternatively you can use `!!`, `df %>% filter(.data[[input$var]] > !!input$min)`. This happens at a slightly different time, and I think is mildly less appealing because it lacks the symmetry of `.data` vs `.env`, and you need to know about `!!`. But it's a fine solution if you know about `!!` already and are happy with the way this works.) 
...
That's fine too, as long as you're aware of all the edge cases where `filter()` behaves differently. (One of the advantages of the tidyverse is not just the routine application, but the careful thought that we've applied to edge cases so that functions work more consistently. I don't want to oversell this, but at the some time, it's easy to forget the quirks of specific base R functions.)
...
(using `one_of()` until tidyselect on CRAN. `one_of()` has the same behaviour as `all_of()` but `all_of()` has a better name.
