...
Technically this is an illusion because you'll still have a single app with a single underlying HTML file, but it's now broken into panels so that only one panel is visible at a time.

Multi-page apps pair particularly well with modules, which you'll learn about in Chapter \@ref(scaling-modules).
Modules allow you to partition up the server function in the same way you partition up the user interface, creating independent components that only interact through well defined connection points.
...
`navlistPanel()` is similar to `tabsetPanel()` but instead of running the tabs titles horizontally, it shows them vertically in a sidebar.
You can also use plain strings to add headings to the list, as shown in Figure \@ref(fig:navlistPanel).
...
Another approach is `navbarPage()` --- it still runs the tab titles horizontally, but you can use `navbarMenu()` to add drop-down menus for an additional level of hierarchy.
...
To continue your app customisation journey, you'll need to learn a little more about the [Bootstrap](https://getbootstrap.com) design framework used by Shiny.
Bootstrap is a collection of HTML conventions, CSS styles, and JS snippets bundled up into a convenient form.
Bootstrap grew out of a framework originally developed for Twitter, but the core development team is now broad and Bootstrap has become one of the most popular CSS frameworks used on the web.
...
Bootstrap is so ubiquitous within the R community that it's easy to get style fatigue: after a while every Shiny app and Rmd file start to look the same.
...
You create a theme with `bslib::bs_theme()` and then apply it by passing it to the `theme` argument of the page layout function:
...
This will automatically determine all of the settings from your app theme as shown by Figure \@ref(fig:thematic).
...
However, if you know the basics of HTML and CSS, it's possible to customise Shiny still further.
Unfortunately HTML and CSS are beyond the scope of this book, but a good place to start are the [HTML basics](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/HTML_basics) and [CSS basics](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/CSS_basics) tutorials made by MDN.

There's no magic behind all of the input, output, and layout functions: they just generate HTML[^action-layout-1].
You can see that HTML by executing UI functions directly in the console:

[^action-layout-1]: The magic that connects inputs and outputs to R happens elsewhere (via javascript) but that's well beyond the scope of this book.
...
(Note that this is the contents of the `<body>` tag; other parts of Shiny take take of generating the `<head>`. If you want to include additional CSS or JS dependencies you'll need to learn `htmltools::htmlDependency()`.
...
In the example below, I use the "raw character constant[^action-layout-2]", `r"()"`, to make it easier to include quotes in the string:

[^action-layout-2]: Introduced in R 4.0.0.
...
See ["Build your entire UI with HTML"](https://shiny.rstudio.com/articles/html-ui.html).
for more details.
