...
`req()` is similar to `stop()`, but instead of throwing an error, it sends a special signal to Shiny telling it that the reactive isn't ready yet, so it should wait longer before updating any downstream inputs.

Possible to use inside of an output, but I recommend only using in `reactive()`s. I think this is good practice because it isolates computation in one place.
...
For example, imagine you've created a Shiny app that allows you to up the name of a state from its abbreviation. What do you want the app to do if you enter a 

```{r}
ui <- fluidPage(
  textInput("abbr", "State abbreviation"),
  plotOutput("map")
)
server <- function(input, output, session) {
  state <- reactive({
    state.name[input$abbr == state.abb]
  })
  output$map <- renderPlot({
    maps::map("state", region = state())
  })
}
```
...
server <- function(input, output, session) {
  state <- reactive({
    req(input$abbr %in% state.abb)
    state.name[input$abbr == state.abb]
  })
  output$name <- renderText(state())
}
```
