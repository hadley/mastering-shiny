...
This analogous to writing good functions in R: user-friendly functions give clear error messages describing what the expected input is and how you have violated those expectations.
...
Now nothing will be displayed until the user has suppled values for both language and name, as shown in Figure \@ref(fig:require-simple2).
...
-   Store the `id` returned by `showNotification()`, and then pass this value to `removeNotification()`.
    ...
    You can learn more about `on.exit()` in [Advanced R](https://adv-r.hadley.nz/functions.html#on-exit){.uri}.
...
This is often hard, particularly since the underlying code is often written in C and it has no way to communicate progres updates to you.
We are working on tools in the [progress package](https://github.com/r-lib/progress) so that packages like dplyr, readr, and vroom will generate progress bars that you can easily forward to Shiny.
...
Adapating the above code to work with Waiter is straightforward.
...
Imagine you have an Shiny app that deletes some files from a directory (or rows in a database etc).
...
-   What should you call the buttons?
    It's best to be descriptive, so avoid yes/no or continue/cancel in favour of recaptiulating the key verb.
...
The primary downside of this technique is that it is substantially more complicated to implement (you have to have a separate "holding cell" that stores the information need to undo the action), and requires regular intervention from the user to avoid accumulating.
