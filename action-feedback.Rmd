...
Notice, however, that while the error message is displayed, the output is still updated. Typically you don't want that because invalid inputs are likely to cause uninformative R errors which you don't want to show to the user. To fix that problem, you need a new tool: `req()`, shorted for "required".
...
When `req()` is not true, it sends a special signal to tell Shiny that the reactive is not complete, so any reactive consumers that follow it in the reactive graph will not be updated. 

We'll take a brief digression to talk about `req()` before we come back to using it in concert with validate.

### Pausing execution with `req()`

It's easiest to understand `req()` by starting with a simple case where it's important outside of validation. An input triggers reactive updates once on app load, before the user does anything. Typically, you'll provide default `value`s in your inputs so that the app starts in a valid state, but sometimes you want to wait until the user has provided some data. This occurs primarily with three controls:

* In `textInput()`, you might want to require the user enter text because
  you've used the default `value = ""`.
...
`cancelOutput`: In certain cases, you might want to leave the last output up. For example, imagine you allow the user to type the name of a dataset. As they're typing, reactives will be firing, and it'd be nice to only change the output once they've typed a complete name.
