...
<-- MCR: Was it a conscious decision to include the session parameter in the server function? This is omitted in other intro learning materials and articles, and is introduced later when relevant. If the book will take the approach of exposing it earlier, it might be worthwhile to align other learning content too. -->
...
<!-- MCR: Should it be XXXOutput to match renderXXX, or is lower case because the function name starts with lower case? I like using *, even with people who might not know about what the * symbol means for string matching. -->
...
MCR: Some ideas for exercises. eval=FALSE in code chunks to prevent them from running.

1. Suppose your friend wants to design an app that allows the user to set a number (`x`) between 1 and 50, and displays the result of multiplying this number by 5. Their code is shown below.
...
Unfortunately the app isn't working as intended, and yields the following result:

```{r, echo = FALSE, out.width = NULL, eval=FALSE}
# Should be eval=TRUE if used in the book
ui <- fluidPage(
...
How would you fix the app code to fulfill the app's desired functionality?

(2) Extend the app from the previous exercise to allow the user to set the value of the multiplier, `y`, so that the app yields the value of `x * y`. The final resul should like something like the following:
...
(3) Enhance the following app using reactive expressions to avoid redundancy in calculating the value of `product`.
...
ui <- fluidPage(
  ...
  "then, (x * y) is",
  textOutput("product"), br(),
  
  "and, (x * y) + 5 is",
  textOutput("product_plus5"), br(),
...
(4) The following app is very similar to one you've seen earlier in the chapter: you select a dataset from a package (this time we're using the HistData package) and prints out a summary and plot of the data. It also follows good practice, and makes use of reactive expressions to avoid redundancy of code. However there is a bug in the code provided below. Can you spot and correct the bug? (Note: Could also show a screenshot here...)
